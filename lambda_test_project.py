...
def lambda_handler(event, context):
    ...
    # Filter only E-Bikes
    data = data[data['vehicle_type']=='E-Bike']
    print(data.head(5))

    cur.execute("CREATE TABLE IF NOT EXISTS public.free_float (id varchar, lat float8, lon float8, is_disabled boolean, is_reserved boolean, provider_id varchar, last_updated timestamp);")

    try:
        for row in data.iterrows():
            #print(row[1])
            cur.execute(f"INSERT INTO public.free_float (id, lat, lon, is_disabled, is_reserved, provider_id, last_updated) \
                          VALUES ('{row[1][0]}', {row[1][1]}, {row[1][2]}, {row[1][3]}, {row[1][4]}, '{row[1][5]}', '{row[1][7]}');")
    except psycopg2.Error as e:
